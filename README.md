...
It needs `NETLIFY_AUTH_TOKEN` and `NETLIFY_SITE_ID` in the environment and uses' curl' to call Netlify's deploy API, without requiring a Node toolchain to install for a single user. It refuses to upload a folder that the build did not just write — publishing a stale build is the one failure that looks exactly like success from the outside.
...
One scene ships, not six. Nothing in the game switches levels — `Restart` reloads the scene it is already in,, and there is no level select — so the other five themes would be megabytes a player has no way toaccessh. The scene list is passed explicitly rather than read from Build Settings, which still has Unity's empty `SampleScene` at index 0; index 0 is what a player boots into.
