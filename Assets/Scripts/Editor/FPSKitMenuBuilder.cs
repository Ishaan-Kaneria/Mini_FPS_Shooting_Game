...
namespace FPSKit.EditorTools
{
    ...
    public static partial class FPSKitMenuBuilder
    {
        ...
        static void BuildScene()
        {
            ...
            if (menu.topBar.rail != null) menu.topBar.rail.transform.SetAsLastSibling();
        ...
        // ------------------------------------------------------------------
        /// <summary>
        /// The ACHIEVEMENTS screen: only the screen itself. What is on it is built at runtime by
        /// <see cref="AchievementsPanel"/> from the kit, so the layout lives in one file.
        /// </summary>
        static void BuildAchievements(RectTransform parent, MainMenuController menu)
        {
            ...
            var shade = UIKit.Panel(parent, "Achievements", UIKit.PanelTone.Background, t);
            shade.borderColor = new Color(0, 0, 0, 0);
            shade.raycastTarget = true;
            UIKit.Fill(shade.rectTransform);
            var content = UIKit.Rect(shade.transform, "Content");
            UIKit.Fill(content);
            // No SafeAreaFitter baked in here: SafeAreaCanvas skips a canvas that already has
            // one, and this one would take the top bar and the rail out of the safe area with
            // it. AchievementsPanel adds its own when it builds, after that has run.
